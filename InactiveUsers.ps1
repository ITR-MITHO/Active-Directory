...
Get-ADuser  -filter * -Properties DisplayName, SamAccountName, LastLogonDate, PasswordLastSet, Manager | Where {$_.LastLogonDate -LT $Date} | Select DisplayName, SamAccountName, LastLogonDate, PasswordLastSet, Manager, Enabled, @{n='OU';e={$_.distinguishedname -replace '^.+?,(CN|OU.+)','$1'}} |
